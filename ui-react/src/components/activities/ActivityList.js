  ...
  return (
    <Paper className={classes.root}>
      ...
      {data && !loading && !error && (
              ...
              ({
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      {last ? <>{last["date"].slice(0, -11)} {last["action"]}  <Link className="edit-link" to={"/"+last["object"].toString().toLowerCase()+"s/" + last["objectId"]}>{last["object"]} </Link>  </>: "no data yet"}
                    </TableCell>
                            ...
                            isEditMode["user_id"]["id"] === contact.id ? (
                                 ...
                                 actions.map(
                                     ({action},i) => {
                                         return (
                                             <div>
                                               {contact.first_name} {actions[i]} {actionDateList[i].slice(0, -11)} <Link className="edit-link" to={"/"+objects[i].toLowerCase()+"s/" + objectIds[i]}>{objects[i]}</Link>
                                             </div>
                                         );
                                     }
                                 )
        ...
        {activitiesCountQueryError && !activitiesCountQueryLoading && <p>Error</p>}
